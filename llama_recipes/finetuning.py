...
def main(**kwargs):
    if "target_modules" in kwargs:
        ...
        if isinstance(target_modules, str):
            ...
            raise TypeError("target_modules must be a string, list, or tuple")
    ...
    print("Training prams: ", train_config)
    print("fsdp parms: ", fsdp_config)
    ...
    if train_config.enable_fsdp and train_config.low_cpu_fsdp:
        ...
        """
        if rank == 0:
            if "Llama" not in train_config.model_name:
            ...
            if "Llama" not in train_config.model_name:
                model = AutoModelForCausalLM.from_pretrained(train_config.model_name, trust_remote_code=True)
            else:
    ...
    tokenizer = AutoTokenizer.from_pretrained(train_config.model_name if train_config.tokenizer_name is None else train_config.tokenizer_name, trust_remote_code=True)
