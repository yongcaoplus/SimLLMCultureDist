...
def compute_kl_loss(input_lens, target_dist, output, criterion=None):
    all_loss = []
    for i, (input_len, target) in enumerate(zip(input_lens, target_dist)):
        # 获取第一 token 的 logit
        first_tok_logit = output.logits[i, input_len - 1]
        # 分割 target
        valid_mask = target != -1
        ...
        # 计算对数概率 KL 散度损失
        probabilities = F.log_softmax(pred_dist, dim=-1)
        golden_dist_tensor = torch.tensor(golden_dist).to(probabilities.device) / 100  # 将 golden_dist 转换为张量并缩放
        loss = F.kl_div(probabilities, golden_dist_tensor, reduction='batchmean')
        # 将损失添加到列表中
        all_loss.append(loss)
    return torch.stack(all_loss).mean()

# wasserstein_distance loss for ablation study
def compute_wasserstein_loss(pred_dist, golden_dist):
    # Wasserstein loss: typically L1 distance between two distributions
    return torch.norm(pred_dist - golden_dist, p=1)

def compute_js_loss(pred_dist, golden_dist):
    # Compute Jensen-Shannon Divergence
    m = 0.5 * (pred_dist + golden_dist)
    js_loss = 0.5 * (F.kl_div(pred_dist.log(), m, reduction='batchmean') + F.kl_div(golden_dist.log(), m, reduction='batchmean'))
    return js_loss

def compute_custom_loss(input_lens, target_dist, output, criterion='wasserstein'):
    all_loss = []
    for i, (input_len, target) in enumerate(zip(input_lens, target_dist)):
        # Get the logit of the first token
        first_tok_logit = output.logits[i, input_len - 1]
        ...
        # Split target into golden distribution and option IDs
        split_index = target.size(-1) // 2
        golden_dist = target[:split_index].tolist()
        golden_option_id = target[split_index:].long().tolist()
        ...
        # Normalize and convert golden_dist to tensor
        golden_dist_tensor = torch.tensor(golden_dist).to(pred_dist.device) / 100
        
        # Compute loss based on the criterion
        if criterion == 'wasserstein':
            loss = compute_wasserstein_loss(pred_dist, golden_dist_tensor)
        elif criterion == 'js':
            loss = compute_js_loss(F.softmax(pred_dist, dim=-1), F.softmax(golden_dist_tensor, dim=-1))
        elif criterion == 'ce':
            loss = F.cross_entropy(pred_dist.unsqueeze(0), torch.tensor(golden_dist).argmax().unsqueeze(0).to("cuda"))
        else:
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    for key in batch.keys():
                        ...
                        try:
                            ...
                            print(f"Error in moving {key} to device")
                    with autocast():
                        ...
                        loss = compute_kl_loss(input_length, target_dist, output)
                        #  ablation study for loss function wasserstein / js
                        # loss = compute_custom_loss(input_length, target_dist, output, criterion='ce')
                    ...
                    # print(model.model.model.layers[0].self_attn.q_proj.lora_A.default.weight)
                    # breakpoint()
                    ...
                    if wandb_run:
                        if not train_config.enable_fsdp or rank==0:
                            wandb_run.log({
                                ...
                                'train/avg_loss': np.mean(all_loss),
                            })
                    all_loss.append(loss.item())
                    pbar.set_description(f"Training Epoch: {epoch+1}/{train_config.num_epochs}, step {step}/{len(train_dataloader)} completed (loss: {np.mean(all_loss):.4f})")
...
def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run):
    ...
    with MemoryTrace() as memtrace:
        for step, batch in enumerate(tqdm(eval_dataloader,colour="green", desc="evaluating Epoch", dynamic_ncols=True)):
            ...
            with torch.no_grad():
                ...
                input_length = batch.pop("input_length")
                outputs = model(**batch)
                loss = compute_kl_loss(input_length, target_dist, outputs)
                #  ablation study for loss function  wasserstein / js
                # loss = compute_custom_loss(input_length, target_dist, outputs, criterion='ce')
                if train_config.save_metrics:
    ...
    if wandb_run:
        wandb_run.log({
                        ...
                        'eval/avg_loss: ': np.mean(val_step_loss)
                    }, commit=False)
